...
contract InheritXPlans is InheritXCore {
    uint256 public constant PLAN_CREATION_FEE = 1 ether;
    ...
    function createInheritancePlan(
        ...
        uint256 totalRequired = assetAmount + PLAN_CREATION_FEE;
        ...
        bool transferSuccess = token.transferFrom(msg.sender, address(this), totalRequired);
        if (!transferSuccess) revert TransferFailed();

        address feeRecipient = feeConfig.feeRecipient;
        if (feeRecipient == address(0)) revert ZeroAddress();

        if (!token.transfer(feeRecipient, PLAN_CREATION_FEE)) {
            revert TransferFailed();
        }

        if (feeAmount > 0) {
            if (!token.transfer(feeRecipient, feeAmount)) {
                revert TransferFailed();
            }
        }
        ...
        uint256 totalFeesForPlan = feeAmount + PLAN_CREATION_FEE;
